...
- `setfacl` 用于设置文件的ACL信息
  ...
  - `-m`：修改ACL规则。
  - `-x`：移除ACL规则。
  - `-R`：递归地应用ACL规则到目录及其子目录。
  - `-d`：用于给目录设置某人ACL规则

  使用案例
  ```shell
  setfacl -m u:user1:rw script.sh #给予user1用户对script.sh的rw权限
  setfacl -x u:user1 script.sh #取消user1用户对script.sh的所有由ACL添加的权限
  setfacl -m g:group1:rw script.sh #给予group1组对script.sh的rw权限
  setfacl -Rm u:user1:rw data/ #给予user1用户对data目录及其下文件的rw权限
  ```
...
- `useradd` 用于创建新用户账号
  ...
  使用案例
  ```shell
  useradd -d /home/user1 -m user1 # 创建用户名为 user1 的用户，指定主目录为 /home/user1
  useradd -g root user1 # 创建用户名为 user1 的用户，将其初始主组设置为 root
  useradd -s /bin/zsh user1 # 创建用户名为 user1 的用户，并设置登录Shell为 /bin/zsh
  useradd -u 1500 baka #创建用户名为 baka 的用户，并设置其uid为1500
  ```
...
- `usermod` 用于修改用户账号的属性
  ...
  常用选项
    ...
    - `-u, --uid UID`：更改用户的UID。
    - `-G, --groups GROUPS`：设置用户所属的附加组（一次可设置多个，每次设置都会覆盖上一次内容）。
    - `-a, --append`：附加用户到附加组，与`-G`同时使用,表示追加。

  使用案例
  ```shell
  usermod -d /home/user1 user1 # 修改 user1 用户的主目录为 /home/user1
  usermod -g group1 user1 # 修改 user1 用户的主要组为 group1
  usermod -s /bin/bash user1 # 修改 user1 用户的登录Shell为 /bin/bash
  usermod -G group2,group3 user1 # 将 user1 用户添加到附加组 group2,group3中，覆盖原有附加组
  usermod -a -G group4 user1 # 将 user1 用户追加到附加组 group2
  ```
