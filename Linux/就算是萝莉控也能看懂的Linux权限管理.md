...
- `sudo` 允许被授权的用户提供自己的密码后以另一个用户的身份执行命令，通常是作为超级用户（root），同时提供日志记录和安全机制。  
  ...
  - `/etc/sudoers`
    ...
    `sudoers` 文件是配置 `sudo` 权限的主要文件，它决定了哪些用户和用户组可以使用 `sudo` 命令以及可以执行哪些命令。
    
    默认该文件是没有写入权限，推荐使用 `visudo` 命令修改；也可以使用 `chmod` 命令修改其权限后，使用 `vi` 等编辑器进行修改（修改结束后记得将权限改回去）

    通常文件具有以下内容
    ![](/img/权限管理/1.png)

    根据发行版不同可能没有 `%admin ALL=(ALL) ALL` `%sudo ALL=(ALL:ALL) ALL`，但都应该会有以下内容
    ```shell
    root ALL=(ALL:ALL) ALL #	root 用户在所有主机上可以以任何用户和任何用户组的身份运行任何命令。
    ```
    以该配置为例，可将其分为四个部分
    ```shell
    ...
    |字段|说明|示例|
    |:---:|:---:|:---:|
    |`user`|	指定用户或用户组（使用 `%group` 表示组），多个用户使用 `,` 分隔|`root`
    |`host`|	指定在哪些主机上有效（通常用 `ALL` 表示所有主机），多个主机使用 `,` 分隔|`ALL` 
    |`run_as`|	指定可以作为哪些用户运行（通常用 `ALL` 表示所有用户），`:`后表示组，多个用户使用 `,` 分隔|`(ALL:ALL)`
    |`command`|	指定允许执行的命令（通常用 `ALL` 表示所有命令），指定命令时需要输入命令完整路径，多个命令使用 `,` 分隔|`ALL`

    特殊关键字

    |关键字|说明|示例|
    |:---:|:---:|:---:|
    |`NOPASSWD`|不需要密码即可使用 `sudo`|`baka ALL=(ALL) NOPASSWD: ALL`
    |`!`|禁止执行某些命令|`baka ALL=(ALL) ALL, !/bin/rm`
